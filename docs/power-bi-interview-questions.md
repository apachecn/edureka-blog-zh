...
> 原文：<https://www.edureka.co/blog/interview-questions/power-bi-interview-questions/>
...
**![gartner-comparison-Power BI Interview Questions-Edureka](img/4c784a3b99bda13a312864cf4dd0da20.png)**

如你所见，与 2016 年相比，2017 年是微软的一个重要时刻，这主要归功于人力资源 BI 的贡献。如果你仍然不确定是否要进军智能商务领域，下面的图片会让你对当前的智能商务市场有所了解:

**![Power BI Trends - Power BI Interview Questions - Edureka](img/a1775d1a179e3e2fe62749c1bca4c9dc.png)**
...
### **①。微软的自助式商业智能解决方案由哪些部分组成？**

**Ans:** 微软有两部分用于自助 BI

 <caption>#### **部分自助 BI**</caption> 
| **Excel BI 工具包** | 允许用户根据报表需求，导入不同来源的数据和模型数据，创建交互式报表。 |
| **力量匕** | 它是一个在线解决方案，使您能够共享使用 Excel BI Toolkit 创建的交互式报告和查询。 |
...
**![Power-BI-Power BI Interview Questions](img/5f7c33a3fab3be55211a0d0ebf0aaeb9.png)**
...
### ![PowerBI_Components - PowerBI Interview Questions - Edureka](img/a9f1036808cd79a67a397f5c60635550.png)
...
![Connectivity Modes - PowerBI Interview Questions - Edureka](img/10f8718b2107519e7c7abf8a60144114.png)

在 PowerBI 中，导入模式是默认模式，因为它是最常用的模式，可以提供快速的性能。它可以集成来自数据源的数据，如图所示。导入的数据存储在磁盘中，并在查询或刷新时完全加载。

**直接查询模式**

**![Direct Query Mode - PowerBI Interview Questions - Edureka](img/4d622e2d6420049d9af3f5d5af8db113.png)**

直接查询模式是另一种使用 Query 导入数据以从预先存在的数据源中检索数据的方法。当数据量太大时，我们使用 DirectQuery 来避免刷新数据，因为这可能需要很长时间。

**复合模式**

![Composite Mode - PowerBI Interview Questions - Edureka](img/b354993dc88cddead2b3492a018d89f2.png)

这种模式是 Import 和 DirectQuery 模式的结合。此模式支持 DirectQuery 不支持的计算表。它提供了最好的导入查询和直接查询模式。

### **11)。Power BI 中的数据存储在哪里？**

首先，PowerBI 使用两个存储库来存储数据:Azure Blob 存储和 Azure SQL 数据库。Azure Blob 存储通常存储用户上传的数据。Azure SQL 数据库存储系统本身的所有元数据和工件。T3T5

### **12)。Power BI 中的构建模块是什么？**
...
### **15)。Power BI 报告中有哪些不同类型的过滤器？**

![filters- Power BI Interview Questions - Edureka](img/5f3df9383f03dabf117ba33ff07ffc7c.png)**Ans:**Power BI 提供多种选项来过滤报表、数据和可视化。以下是过滤器类型列表。

*   **可视化级过滤器:** 这些过滤器只对单个可视化起作用，减少了可视化可以看到的数据量。此外，可视化级别的过滤器可以过滤数据和计算。
...
## **![PowerBI Dashboard - PowerBI Interview Questions - Edureka](img/e0ef02a72fb56b7bff07843df474af0d.png)**
...
*![DAX - Power BI Interview Questions - Edureka](img/73b9aefcff99837da767f8a6385ca410.png)*
...
### **36)。CALCULATE 和 CALCULATETABLE 函数有什么特殊或独特之处？**
...
### **39)。你如何通过 DAX 针对非标准日历创建尾随 X 月指标？**
...
### **45)。在 power pivot 的数据模型中，两个表之间可以有多个活动关系吗？**
...
### **54)。当 Power Pivot 可以从最常用的来源导入数据时，为什么我们需要 Power Query？**
...
![Example of correct table format - Power BI Interview Questions -Edureka](img/d1c346794e83e786a13d7a561f9b2b14.png)
...
### **69)。Excel experience 可以通过哪些方式与 Power BI 结合使用？**
...
*   用户可以在 Excel 中管理图块——刷新、删除。
*   必须从服务中的仪表板上删除固定项目(在 Excel 中删除只会删除连接)。
*   Power BI Publisher for Excel 还可用于从 Excel 连接到 Power BI 服务中托管的数据集。
*   生成一个 Excel 数据透视表，并连接到 Azure 中的数据(ODC 文件)。

发布者在本地机器上安装所有必要的驱动程序以建立连接。

### **75)。Power BI 数据集、报告和仪表板之间有什么区别？**

**Ans:****Dataset:**用于创建报告和可视化/平铺的源。
...
*   某些 SSRS 报表项目(如图表)可以固定到 Power BI 仪表板。
*   点击 Power BI 仪表盘中的图标，用户将看到 SSRS 报告。
*   创建订阅以保持仪表板标题刷新。
